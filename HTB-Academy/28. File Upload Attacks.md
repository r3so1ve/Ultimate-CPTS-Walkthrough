...
I found a form for submitting with uploading functionality:
...
So we can see that files get uploaded to: /user_


So I started to fuzz for valid extensions and found that .phar is working:


So I uploaded a random .jpg picture, changed the magic bytes to jpeg and Content-Type to: jpeg and filename to: image.phar.jpg and forwarded the request, and got this:


So now we need to go to:
```
http://IP:PORT/contact/user_fd_fdf/image.phar.jpg?cmd=id
```

To make sure that everything is working

Then we need to list the files: 
```
http://IP:PORT/contact/user_fd_fdf/image.phar.jpg?cmd=ls+/
```

And after that we locate the flag.txt so we simply cat it
```
http://IP:PORT/contact/user_fd_fdf/image.phar.jpg?cmd=cat+/flag.txt
```
