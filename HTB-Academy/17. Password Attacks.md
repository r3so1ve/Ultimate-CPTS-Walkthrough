...
|`<username>:`|`<password>:`|`<uid>:`|`<gid>:`|`<comment>:`|`<home directory>:`|`<cmd executed after logging in>`|
 ...
 The /etc/passwd file, contains information about Unix/Linux system users. This file does not store passwords explicitly, but provides information about users such as their names, user IDs (UIDS), group IDs (GIDs), home directory, and the shell used to log in. This is what the lines in the /etc/passwd file mean: root:x:0:0:root:/root:/bin/bash root: user name x: The password is stored encrypted in the /etc/shadow file 0:0: UID and GID (both equal to 0 for superuser) root: description or comment /root: the user's home directory /bin/bash: the command shell used to log in
