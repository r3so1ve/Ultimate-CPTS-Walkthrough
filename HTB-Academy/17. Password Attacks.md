...
### Основные различия между **keytab** и **ccache**:

| Характеристика           | **Keytab**                                                | **CCache**                                                  |
| ------------------------ | --------------------------------------------------------- | ----------------------------------------------------------- |
| **Содержание**           | Содержит зашифрованные ключи (ключи Kerberos)             | Содержит билеты Kerberos (TGT и сервисные билеты)           |
| **Используется**         | Программами и сервисами для автоматической аутентификации | Пользователями и процессами для временного хранения билетов |
| **Назначение**           | Аутентификация сервиса или компьютера без ввода пароля    | Временное хранение билетов для доступа к ресурсам           |
| **Срок действия**        | Не имеет срока действия                                   | Билеты имеют срок действия (TGT и сервисные билеты)         |
| **Генерация**            | Создается администратором или службой Kerberos            | Создается после успешной аутентификации пользователя        |
| **Пример использования** | Аутентификация сервиса в домене Active Directory          | Доступ пользователя к файловым ресурсам после `kinit`       |
## Commands 
|**Command**|**Description**|
|---|---|
