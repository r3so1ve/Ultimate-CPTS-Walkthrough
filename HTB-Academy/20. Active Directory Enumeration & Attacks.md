...
So this is kind of a tricky one. First we need to run the following command, which will search for Users With Specific Attributes Set (ADS_UF_ACCOUNTDISABLE) so to see the disabled account (we need to put 2 in the end of the userAccountControl instead of 32 which was in the example) refer to the image above 21 question to see the detailed explana
```c
