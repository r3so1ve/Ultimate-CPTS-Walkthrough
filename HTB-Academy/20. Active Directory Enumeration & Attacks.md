...
As we are sshed to target we can use crackmapexec with grep option to filter only successful attempt of password spraying for findig user starting with "s" with password Welcome1, also we can use kerbrute for that.
```c
