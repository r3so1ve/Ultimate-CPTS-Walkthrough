...
So we see 2 internal ip addresses, let's start with the first one. We can use rpcclient with null session, enum4linux, or ldapsearch to find the password policy requirement about minimum length of the password in 
```c
```