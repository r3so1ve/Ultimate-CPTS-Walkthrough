...
Overall Dsquery is a command-line tool that can be utilized to find Active Directory objects. Combined with LDAP filtering we can now display the accounts that are disabled. In our case we need to find the one which belongs to Administrator group
```c
