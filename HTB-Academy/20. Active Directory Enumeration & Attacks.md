...
For that we need to simply read the section about PowerView and actually familiarize ourselves with it's functions.
In our case it is `Test-AdminAccess`
...
19. Run Snaffler and hunt for a readable web config file. What is the name of the user in the connection string within the file?
 ...
 [Snaffler](https://github.com/SnaffCon/Snaffler) is a tool that can help us acquire credentials or other sensitive data in an Active Directory environment. Snaffler works by obtaining a list of hosts within the domain and then enumerating those hosts for shares and readable directories. We may find passwords, SSH keys, configuration files, or other data that can be used to further our access. Snaffler color codes the output for us and provides us with a rundown of the file types found in the shares.
```c
...
From the previous output of snaffer we also found the password of `sa` which is `ILFREIGHTDB01!` and that's the answer
...
![](https://i.imgur.com/F8IG8tE.png)


21. Enumerate the host's security configuration information and provide its AMProductVersion.

First RDP to target as usual
```c
xfreerdp /u:htb-student /p:Academy_student_AD! /v:10.129.7.236 /cert-ignore /bpp:8 /network:modem /compression -themes -wallpaper /clipboard /audio-mode:1 /auto-reconnect -glyph-cache /dynamic-resolution
```

After that we need to open powershell and run the following command
```c
...
22.  What domain user is explicitly listed as a member of the local Administrators group on the target host?
...
23. Utilizing techniques learned in this section, find the flag hidden in the description field of a disabled account with administrative privileges. Submit the flag as the answer.

So this is kind of a tricky one. First we need to run the following command, which will search for Users With Specific Attributes Set (ADS_UF_ACCOUNTDISABLE) so to see the disabled account (we need to put 2 in the end of the userAccountControl instead of 32 which was in the example) refer to the image above 21 question to see the detailed explanation.
```c
PS C:\Windows\system32> dsquery * -filter "(&(objectCategory=person)(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=2))" -attr distinguishedName userAccountControl
  distinguishedName                                                                                   userAccountControl
  CN=Guest,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                                                  66082
  CN=krbtgt,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                                                 514
  CN=Betty Ross,OU=IT Admins,OU=IT,OU=HQ-NYC,OU=Employees,OU=Corp,DC=INLANEFREIGHT,DC=LOCAL                    66050
  CN=Jessica Ramsey,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                                         546
  CN=Jessica Msexchapproval 1F05a927-3Be2-4Fb9-Aa03-B59fe3b56f4c,CN=Users,DC=INLANEFREIGHT,DC=LOCAL            514
  CN=Jessica Systemmailbox Bb558c35-97F1-4Cb9-8Ff7-D53741dc928c,CN=Users,DC=INLANEFREIGHT,DC=LOCAL             514
  CN=Jessica Msexchdiscovery E0dc1c29-89C3-4034-B678-E6c29d823ed9,CN=Users,DC=INLANEFREIGHT,DC=LOCAL           514
  CN=Jessica Msexchdiscoverymailbox D919ba05-46A6-415F-80Ad-7E09334bb852,CN=Users,DC=INLANEFREIGHT,DC=LOCAL    514
  CN=Jessica Migration.8F3e7716-2011-43E4-96B1-Aba62d229136,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                 514
  CN=Jessica Federatedemail.4C1f4d8b-8179-4148-93Bf-00A95fa1e042,CN=Users,DC=INLANEFREIGHT,DC=LOCAL            514
  CN=Jessica Systemmailbox{D0e409a0-Af9b-4720-92Fe-Aac869b0d201},CN=Users,DC=INLANEFREIGHT,DC=LOCAL            514
  CN=Jessica Systemmailbox{2Ce34405-31Be-455D-89D7-A7c7da7a0daa},CN=Users,DC=INLANEFREIGHT,DC=LOCAL            514
  CN=Jessica Systemmailbox 8Cc370d3-822A-4Ab8-A926-Bb94bd0641a9,CN=Users,DC=INLANEFREIGHT,DC=LOCAL             514
```

Overall Dsquery is a command-line tool that can be utilized to find Active Directory objects. Combined with LDAP filtering we can now display the accounts that are disabled. In our case we need to find the one which belongs to Administrator group, so it is Betty Ross, from the below output. However to use `net` commands for this woman, we need to know her username, as from dsquery command output below we see that her name consists of 2 words: Betty Ross. So we need to Get the sAMAccountName for Betty Ross. Often, **Net User** requires the “Pre–Windows 2000” logon name (the **sAMAccountName**) rather than the full display name. So we can run
```c
PS C:\Windows\system32> dsquery user -name "Betty Ross" | dsget user -samid
  samid
  bross
dsget succeeded
```

We see that `bross` is short variant of Betty Ross, so finally we can use `net` command to get info about this user in the domain as well as get the flag
```c
PS C:\Windows\system32> net user bross /domain
...
Password last set            10/27/2021 9:37:07 AM
Password expires             Never
Password changeable          10/28/2021 9:37:07 AM
...
24. Retrieve the TGS ticket for the SAPService account. Crack the ticket offline and submit the password as your answer.


25. What powerful local group on the Domain Controller is the SAPService user a member of?
