...
Overall Dsquery is a command-line tool that can be utilized to find Active Directory objects. Combined with LDAP filtering we can now display the accounts that are disabled. In our case we need to find the one which belongs to Administrator group, so it is Betty Ross, from the below output. However to use `net` commands for this woman, we need to know her username, as from dsquery command output below we see that her name consists of 2 words: Betty Ross